...
export function Profiles(): JSX.Element {
  ...
  const [montageCaps, setMontageCaps] = useState<MontageCapabilities | null>(null)
  ...
  const montageEnabled = !!settings.montage?.enabled
  const montageFootageSource = config.styleConfig.montageFootageSource
  const montageRuntime = config.styleConfig.montageRuntime
  // OpenMontage options are gated on both settings.montage.enabled and a successful capability probe.
  const omFootageReady = montageEnabled && !!montageCaps?.available
  const omRemotionReady = montageEnabled && !!montageCaps?.renderEngines.remotion
  const omHyperframesReady = montageEnabled && !!montageCaps?.renderEngines.hyperframes
  ...
  useEffect(() => {
    if (!settings.montage?.enabled) { setMontageCaps(null); return }
    let cancelled = false
    void window.api.montage.capabilities().then((caps) => { if (!cancelled) setMontageCaps(caps) }).catch(() => { if (!cancelled) setMontageCaps(null) })
    return () => { cancelled = true }
  }, [settings.montage?.enabled])
  useEffect(() => {
  ...
  return <ScreenPad>
    ...
    {view === 'setup' ? <>
      ...
      {stage === 2 && <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {montageEnabled && <Panel><SectionLabel>OpenMontage footage and composition (beta)</SectionLabel><div className="automation-help">{montageCaps ? (montageCaps.available ? 'Route batch footage and composition through the external OpenMontage app. Runtimes that are not installed are disabled below.' : (montageCaps.error || 'OpenMontage is enabled but not reachable. Check the OpenMontage path and Python interpreter in Settings.')) : 'Checking OpenMontage availability…'}</div><div className="automation-config-grid" style={{ marginTop: 12 }}><Field label="Footage source" hint="Native uses your saved B-roll pool. OpenMontage adds public-domain archives and extra stock providers."><select value={montageFootageSource} onChange={(event) => updateStyle('montageFootageSource', event.target.value as AutomationMontageFootageSource)} style={input}><option value="native">Native · saved B-roll pool</option><option value="archives" disabled={!omFootageReady}>OpenMontage · public-domain archives</option><option value="stock" disabled={!omFootageReady}>OpenMontage · extra stock (Pexels/Unsplash/Pixabay)</option></select></Field><Field label="Composition runtime" hint="Native uses the MES compositor. Remotion and HyperFrames render through OpenMontage."><select value={montageRuntime} onChange={(event) => updateStyle('montageRuntime', event.target.value as AutomationMontageRuntime)} style={input}><option value="native">Native · MES compositor</option><option value="remotion" disabled={!omRemotionReady}>OpenMontage · Remotion</option><option value="hyperframes" disabled={!omHyperframesReady}>OpenMontage · HyperFrames</option></select></Field></div>{montageCaps?.runtimeWarnings?.length ? <div className="automation-help" style={{ marginTop: 8 }}>{montageCaps.runtimeWarnings.join(' · ')}</div> : null}</Panel>}
